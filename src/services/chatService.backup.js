    ...
    getContacts: async () => {
        try {
            ...
            const response = await axios.get(
                'http://localhost:8080/chatapp/api/contacts',
                config
            );
            return response.data;
        } catch (error) {
...
export default chatService;           const response = await chatApi.getConversationWithIdAccount();
            return response.data;
        } catch (error) {
            console.error('Error fetching conversations:', error);
            throw error;
